...
1.  Pythonをインストールします。 どの Python バージョンがサポートされているかは、 [setup.py](https://github.com/Qiskit/qiskit/blob/main/setup.py) ファイルの要件を確認してください。  ダウンロード方法については、 [Pythonビギナーズガイド](https://wiki.python.org/moin/BeginnersGuide/Download) を参照してください。

    [Python仮想環境](https://docs.python.org/3.10/tutorial/venv.html) を使用して、Qiskitを他のアプリケーションから分離することをお勧めします。  また、量子コンピューターとの対話には、 [Jupyter](https://jupyter.org/install) 開発環境を使用することをお勧めします。

    1.  Pythonのみをインストールした最小環境を作ります。
...
2.  [pipをインストールします](https://pip.pypa.io/en/stable/installation/) 。
...
3.  以下のパッケージをインストールします。
    ...
    ```
      ...
      `pip list` を実行すると、仮想環境で有効なパッケージが表示されます。

      視覚化機能やJupyterノートブックを使用する場合は、Qiskitの視覚化サポートを追加してインストールすることをお勧めします。 **zsh ユーザー** は `'qiskit[visualization]'` をシングルクォートで囲む必要があります。
...
ローカルで作業し、Qiskitに組み込まれたシミュレーターを使用する予定であれば、インストールは完了です。 If you want to run jobs on IBM Quantum systems, next [select an access channel](setup-channel) and finish your setup.
